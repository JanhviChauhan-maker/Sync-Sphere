...
class AIAnalyticsEngine:
    ...
    async def get_ai_analytics(self, org_id: str) -> Dict[str, Any]:
        ...
        # target_today must be the ACTUAL current day, not a fabricated/hardcoded past date
        ...
        for e in recent_executions:
            ...
            if e.created_at and e.created_at >= target_today:
                is_openrouter = (e.provider_name or "").lower() == "openrouter"
                is_success = t_tok > 0
                if is_openrouter and is_success:
        ...
        # ---------------------------------------------------------
        # OVERRIDE WITH REAL OPENROUTER API USAGE
        # ---------------------------------------------------------
        import httpx
        from syncsphere.core.config.settings import settings
        
        try:
            key_val = settings.ai.llm_api_key.get_secret_value() if hasattr(settings.ai.llm_api_key, "get_secret_value") else settings.ai.llm_api_key
            async with httpx.AsyncClient(timeout=10.0) as client:
                r = await client.get("https://openrouter.ai/api/v1/auth/key", headers={"Authorization": f"Bearer {key_val}"})
                if r.status_code == 200:
                    key_data = r.json().get("data", {})
                    # OpenRouter /auth/key endpoint provides exact usage_daily metrics (cost)
                    # For token counts, OpenRouter does not expose them globally for free tier here,
                    # so we reflect the cost directly as the "REAL" usage data from OpenRouter.
                    real_cost = key_data.get("usage_daily", 0.0)
                    total_cost = real_cost
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"Failed to fetch real OpenRouter usage: {e}")
